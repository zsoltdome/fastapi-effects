...
from fastapi_mergen.postgres import PollingRelay, RelayConfig

from .app.db import start_database, stop_database
from .app.mergen_config import mergen


def _relay_database_url() -> str:
    value = os.getenv("MERGEN_EXAMPLE_RELAY_DATABASE_URL")
    if not value:
        raise RuntimeError("MERGEN_EXAMPLE_RELAY_DATABASE_URL is required by the relay.")
    return value
