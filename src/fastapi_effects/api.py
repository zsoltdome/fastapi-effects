...
from fastapi_effects.core.routing import (
    HandlerRouteBuilder,
    RouteRegistry,
    RouteSpecification,
...
class EffectContext(Generic[PayloadT]):
    ...
    def __post_init__(self) -> None:
        if not isinstance(self.event, Event):
            raise FastAPIEffectsConfigurationError("Effect context event must be an Event.")
        if not isinstance(self.principal, Principal):
            raise FastAPIEffectsConfigurationError("Effect context principal must be a Principal.")
        if not isinstance(self.delivery_id, UUID):
            raise FastAPIEffectsConfigurationError("Effect context delivery_id must be a UUID.")
        if not isinstance(self.route_key, str) or not _ROUTE_KEY.fullmatch(self.route_key):
            raise FastAPIEffectsConfigurationError("Effect context route_key is invalid.")
        if not _is_positive_integer(self.route_version):
            raise FastAPIEffectsConfigurationError("Effect context route_version must be positive.")
        if not _is_positive_integer(self.attempt_number):
            raise FastAPIEffectsConfigurationError(
                "Effect context attempt_number must be positive."
            )
        if self._application_session_provider is not None and not callable(
            self._application_session_provider
        ):
            raise FastAPIEffectsConfigurationError("Application session provider must be callable.")
...
class FastAPIEffects:
    ...
    def __init__(
        ...
        *,
        ...
        if handler_session_provider is not None and not callable(handler_session_provider):
            raise FastAPIEffectsConfigurationError("handler_session_provider must be callable.")
    ...
    def store_name(self) -> str | None:
        ...
        if not isinstance(name, str) or not name:
            raise FastAPIEffectsConfigurationError("Effect store name must be a non-empty string.")
        return name
    ...
    def handler_executor(self) -> Any:
        ...
        from fastapi_effects.handlers.executor import HandlerExecutor
    ...
    ) -> Callable[..., Awaitable[FastAPIEffectsUnitOfWork]]:
    ...
    def _prepare_authorization_snapshots(self) -> None:
        ...
        for spec in service_routes:
            ...
            if policy is None:
                raise FastAPIEffectsConfigurationError("Service-policy route has no policy name.")
            try:
            ...
            if capabilities is None:
                raise FastAPIEffectsConfigurationError("Named service policy is not registered.")
