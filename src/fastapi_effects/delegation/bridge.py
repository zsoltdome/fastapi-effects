...
class TrustedCallerMetadata:
    ...
    def __post_init__(self) -> None:
        if not isinstance(self.principal, Principal):
            raise FastAPIEffectsConfigurationError("Trusted caller principal is invalid.")
        if not isinstance(self.authentication_source, str) or not _SOURCE.fullmatch(
            ...
            raise FastAPIEffectsConfigurationError(
                "Trusted caller authentication source is invalid."
            )
        if (
            ...
            raise FastAPIEffectsConfigurationError("Trusted caller delegation depth is invalid.")
    ...
    def __init__(
        ...
        *,
        ...
        if not isinstance(forwarded_headers, frozenset):
            raise FastAPIEffectsConfigurationError("Delegation bridge policy is invalid.")
        ...
        if (
            ...
            raise FastAPIEffectsConfigurationError("Delegation header name is invalid.")
        self._issuer = issuer
    ...
    def prepare(
        ...
        *,
        ...
        if not isinstance(requested_scopes, frozenset):
            raise FastAPIEffectsConfigurationError("Delegation requested scopes are invalid.")
        effective = requested_scopes & principal.scopes & self._allowed_scopes
        if effective != requested_scopes:
            raise FastAPIEffectsConfigurationError(
                "Delegation component policy denied requested scopes."
            )
        safe_headers: dict[str, str] = {}
        for key, value in (inbound_headers or {}).items():
            if not isinstance(key, str) or not isinstance(value, str):
                raise FastAPIEffectsConfigurationError("Delegation inbound header is invalid.")
            normalized = key.lower()
            if normalized not in self._forwarded_headers:
                continue
            if len(value) > 1024 or any(character in value for character in "\r\n\x00"):
                raise FastAPIEffectsConfigurationError("Delegation forwarded header is invalid.")
