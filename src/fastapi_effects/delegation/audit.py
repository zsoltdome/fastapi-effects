...
class DelegationAuditEvent:
    ...
    def __post_init__(self) -> None:
        if (
            ...
            raise FastAPIEffectsConfigurationError("Delegation audit time must be timezone-aware.")
        if not isinstance(self.outcome, DelegationAuditOutcome):
            raise FastAPIEffectsConfigurationError("Delegation audit outcome is invalid.")
        for value in (
            self.subject_id,
            self.actor_id,
            self.client_id,
            ...
            if value is not None and (not isinstance(value, str) or not _VALUE.fullmatch(value)):
                raise FastAPIEffectsConfigurationError("Delegation audit identity is invalid.")
        if self.tenant_id is not None and not isinstance(self.tenant_id, UUID):
            raise FastAPIEffectsConfigurationError("Delegation audit tenant is invalid.")
        if self.token_id is not None and not isinstance(self.token_id, UUID):
            raise FastAPIEffectsConfigurationError("Delegation audit token ID is invalid.")
        if (
            ...
            raise FastAPIEffectsConfigurationError("Delegation audit scopes are invalid.")
        ...
        if self.delegation_depth is not None and (
            ...
            raise FastAPIEffectsConfigurationError("Delegation audit depth is invalid.")
