...
from fastapi_effects.core.context import bind_principal, reset_principal
from fastapi_effects.core.event import Event, EventRecord
from fastapi_effects.core.identity import DedupeIdentity, UUIDSource
from fastapi_effects.core.principal import Principal
from fastapi_effects.core.protocols import (
    Clock,
    EffectRouteProvider,
    EffectStore,
    UUIDGenerator,
...
from fastapi_effects.idempotency.store import CommandAcquisition, CommandStore

_SESSION_MARKER = "fastapi_effects.active_uow"
...
class CommandContext:
    ...
    def __post_init__(self) -> None:
        if not isinstance(self.session, AsyncSession):
            raise FastAPIEffectsConfigurationError(
                "CommandContext session must be an AsyncSession."
            )
        if not isinstance(self.principal, Principal):
            raise FastAPIEffectsConfigurationError("CommandContext principal must be a Principal.")
        if not isinstance(self.identity, CommandIdentity):
            raise FastAPIEffectsConfigurationError("CommandContext identity is invalid.")
        if not isinstance(self.fingerprint, RequestFingerprint):
            raise FastAPIEffectsConfigurationError("CommandContext fingerprint is invalid.")
    ...
    async def __aenter__(self) -> CommandContext:
        if self._transaction is not None or self.session.info.get(_SESSION_MARKER) is not None:
            raise FastAPIEffectsConfigurationError(
                "Nested FastAPIEffects transaction ownership is not allowed."
            )
        ...
        if self.identity.tenant_id != self.principal.tenant_id:
            raise FastAPIEffectsConfigurationError(
                "Command identity tenant does not match its principal."
            )
    ...
    async def __aexit__(
        ...
        if missing:
            exc_value = FastAPIEffectsConfigurationError(
    ...
    async def complete(self, response: CapturedResponse | Response) -> CapturedResponse:
        ...
        if acquisition.replayed:
            raise FastAPIEffectsConfigurationError("A replayed command cannot be completed again.")
        if self._completed:
            raise FastAPIEffectsConfigurationError("Command completion may only occur once.")
        captured = (
    ...
    async def emit(
        ...
        *,
        ...
        if acquisition.replayed:
            raise FastAPIEffectsConfigurationError(
                "A replayed command cannot emit new effect intent."
            )
        if self.effect_store is None:
            raise FastAPIEffectsConfigurationError("No transactional effect store is configured.")
    ...
    def replay_response(self) -> Response:
        ...
        if response is None:
            raise FastAPIEffectsConfigurationError("The command does not have a replay response.")
    ...
    def _require_acquisition(self) -> CommandAcquisition:
        if self._transaction is None or self._acquisition is None:
            raise FastAPIEffectsConfigurationError("CommandContext is not active.")
...
def _unique_routes(
    ...
    for route in routes:
        ...
        if identity in unique:
            raise FastAPIEffectsConfigurationError("Duplicate static and dynamic delivery route.")
