...
class CommandIdentity:
    ...
    def __post_init__(self) -> None:
        if not isinstance(self.tenant_id, UUID):
            raise FastAPIEffectsConfigurationError("Command tenant identity is invalid.")
        if not isinstance(self.route_id, str) or not _ROUTE.fullmatch(self.route_id):
            raise FastAPIEffectsConfigurationError("Command route identity is invalid.")
        if not isinstance(self.method, str) or not _METHOD.fullmatch(self.method):
            raise FastAPIEffectsConfigurationError("Command method is invalid.")
        if not isinstance(self.key_digest, bytes) or len(self.key_digest) != 32:
            raise FastAPIEffectsConfigurationError("Command key digest is invalid.")
        ...
        *,
        ...
        if isinstance(key, str):
            try:
                ...
                raise FastAPIEffectsConfigurationError("Idempotency key is invalid UTF-8.") from exc
        elif isinstance(key, bytes):
            raw = key
        else:
            raise FastAPIEffectsConfigurationError("Idempotency key must be text or bytes.")
        if not 1 <= len(raw) <= 512 or any(item < 0x20 or item == 0x7F for item in raw):
            raise FastAPIEffectsConfigurationError("Idempotency key is invalid or oversized.")
        return cls(
            tenant_id=tenant_id,
            route_id=route_id,
            method=method.upper(),
