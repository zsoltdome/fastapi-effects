...
class RequestFingerprint:
    ...
    def __post_init__(self) -> None:
        if (
            ...
            raise FastAPIEffectsConfigurationError("Command request fingerprint is invalid.")
...
def fingerprint_request(
    *,
    ...
    if not isinstance(raw_query, bytes) or len(raw_query) > 16 * 1024:
        raise FastAPIEffectsConfigurationError("Command query representation is oversized.")
    ...
    if mode is BodyFingerprintMode.CANONICAL_JSON:
        ...
        body_identity = b"fastapi_effects:command-raw:v1\n" + body
    else:
        raise FastAPIEffectsConfigurationError("Command body fingerprint mode is invalid.")
...
def _path_parameters(value: Mapping[str, str]) -> dict[str, str]:
    if len(value) > 64:
        raise FastAPIEffectsConfigurationError("Command path parameters are oversized.")
    result: dict[str, str] = {}
    for key, item in value.items():
        if (
            ...
            raise FastAPIEffectsConfigurationError("Command path parameter is invalid.")
        result[key] = item
    return result
...
def _headers(value: Mapping[str, str] | Sequence[tuple[str, str]]) -> dict[str, str]:
    ...
    for raw_name, raw_value in items:
        if not isinstance(raw_name, str):
            raise FastAPIEffectsConfigurationError("Command representation header is invalid.")
        name = raw_name.lower()
        if (
...
def _media_type(value: str | None) -> str | None:
    ...
    if (
        ...
        raise FastAPIEffectsConfigurationError("Command media type is invalid.")
