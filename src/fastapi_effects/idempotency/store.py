...
from fastapi_effects.idempotency.models import CommandIdentity, CommandRow, CommandState
...
class CommandAcquisition:
    ...
    def __post_init__(self) -> None:
        if self.replayed != (self.response is not None):
            raise FastAPIEffectsConfigurationError("Command acquisition response state is invalid.")
    ...
    async def prune(
        ...
        *,
        ...
        if session.in_transaction():
            raise FastAPIEffectsConfigurationError("Command pruning requires an idle session.")
        if before.tzinfo is None or before.utcoffset() is None:
            raise FastAPIEffectsConfigurationError("Command pruning cutoff must be timezone-aware.")
        if not 1 <= batch_size <= 10_000:
            raise FastAPIEffectsConfigurationError("Command pruning batch size is invalid.")
        async with session.begin():
            deleted = await session.scalar(
                text("SELECT fastapi_effects.prune_commands(:cutoff, :batch_size)"),
                {"cutoff": before, "batch_size": batch_size},
            )
        if not isinstance(deleted, int):
            raise FastAPIEffectsConfigurationError("Command pruning returned an invalid result.")
        if deleted:
            record_safely(
                self._event_sink,
                RuntimeEvent(
...
def _validate_request(
    ...
    if now.tzinfo is None or now.utcoffset() is None:
        raise FastAPIEffectsConfigurationError("Command timestamp must be timezone-aware.")
    if ttl < timedelta(seconds=1) or ttl > timedelta(days=365):
        raise FastAPIEffectsConfigurationError("Command expiry interval is invalid.")


def _require_transaction(session: AsyncSession) -> None:
    if not session.in_transaction():
        raise FastAPIEffectsConfigurationError("Command storage requires an active transaction.")


def _advisory_key(identity: CommandIdentity) -> int:
    digest = hashlib.sha256(
        b"fastapi_effects:command-lock:v1\0"
...
def _response_from_row(row: CommandRow) -> CapturedResponse:
    if (
        ...
        raise FastAPIEffectsConfigurationError("Completed command response is unavailable.")
