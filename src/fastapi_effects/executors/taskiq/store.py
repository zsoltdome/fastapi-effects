...
from fastapi_effects.core.delivery import AttemptOutcome, DeliveryState
...
from fastapi_effects.executors.protocols import HandoffState
from fastapi_effects.executors.taskiq.envelope import TaskiqHandoffEnvelope, stable_task_id
...
class TaskiqHandoffStore:
        ...
        *,
        ...
        async with session.begin():
            ...
            if row is None or (inserted is None and row.state != HandoffState.PREPARED.value):
                raise FastAPIEffectsConfigurationError("Taskiq handoff preparation conflicted.")
    ...
    async def claim_execution(
        ...
        *,
        ...
        if execution_timeout <= timedelta(0) or execution_timeout > timedelta(hours=1):
            raise FastAPIEffectsConfigurationError("Taskiq execution timeout is invalid.")
        async with session.begin():
            row = await self._locked_for_envelope(session, envelope)
            ...
            if not isinstance(retry, dict):
                raise FastAPIEffectsConfigurationError("Taskiq retry snapshot is invalid.")
    ...
    async def _locked_for_envelope(
        ...
        if row is None:
            raise FastAPIEffectsConfigurationError("Taskiq handoff envelope was rejected.")
        return row
...
async def _claim_for_row(session: AsyncSession, row: TaskiqHandoffRow) -> ClaimedDelivery:
    ...
    if event is None:
        raise FastAPIEffectsConfigurationError("Taskiq handoff event no longer exists.")
...
def _require_idle(session: AsyncSession) -> None:
    if session.in_transaction():
        raise FastAPIEffectsConfigurationError("Taskiq handoff operation requires an idle session.")
