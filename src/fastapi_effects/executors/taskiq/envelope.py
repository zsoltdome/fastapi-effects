...
class TaskiqHandoffEnvelope:
    ...
    def __post_init__(self) -> None:
        for value in (
            ...
            if not isinstance(value, UUID):
                raise FastAPIEffectsConfigurationError("Taskiq handoff identity is invalid.")
        if (
            not isinstance(self.task_id, str)
            or not self.task_id.startswith("fastapi_effects_")
            or len(self.task_id) > 128
        ):
            raise FastAPIEffectsConfigurationError("Taskiq task identity is invalid.")
        if self.schema_version != 1:
            raise FastAPIEffectsConfigurationError(
                "Taskiq handoff envelope revision is unsupported."
            )
    ...
    def from_dict(cls, value: dict[str, object]) -> TaskiqHandoffEnvelope:
        ...
        if set(value) != expected:
            raise FastAPIEffectsConfigurationError("Taskiq handoff envelope fields are invalid.")
        try:
            ...
            raise FastAPIEffectsConfigurationError("Taskiq handoff envelope is invalid.") from exc


def stable_task_id(attempt_id: UUID) -> str:
    if not isinstance(attempt_id, UUID):
        raise FastAPIEffectsConfigurationError("Taskiq attempt identity must be a UUID.")
    return f"fastapi_effects_{attempt_id.hex}"
