...
from fastapi_effects.conformance.safety import JsonValue, scan_for_secret_values
from fastapi_effects.conformance.scenarios import SCENARIOS, Scenario
...
class RunnerConfiguration:
    ...
    def __post_init__(self) -> None:
        if not isinstance(self.profile, CertificationProfile):
            raise FastAPIEffectsConfigurationError("Runner profile is invalid.")
        if self.check_timeout_seconds is not None and (
            ...
            raise FastAPIEffectsConfigurationError(
                "Runner check timeout must be within 0 and 300 seconds."
            )
        if (
        ...
        if not isinstance(self.fail_fast, bool):
            raise FastAPIEffectsConfigurationError("Runner fail_fast must be a boolean.")
        if not isinstance(self.secret_canaries, tuple):
            raise FastAPIEffectsConfigurationError("Runner secret canaries must be a tuple.")
        if len(self.secret_canaries) > 32:
            raise FastAPIEffectsConfigurationError("Runner accepts at most 32 secret canaries.")
        for canary in self.secret_canaries:
            if not isinstance(canary, str) or not canary or len(canary) > 4096:
                raise FastAPIEffectsConfigurationError("Runner secret canary is invalid.")
