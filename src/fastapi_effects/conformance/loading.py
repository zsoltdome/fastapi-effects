...
async def load_driver(specification: str) -> BoundaryDriver:
    ...
    if match is None or "__" in specification:
        raise FastAPIEffectsConfigurationError("Conformance driver specification is invalid.")
    try:
        ...
        raise FastAPIEffectsConfigurationError("Conformance driver could not be loaded.") from exc
    if not isinstance(value, BoundaryDriver):
        raise FastAPIEffectsConfigurationError("Loaded object is not a BoundaryDriver.")
    return value
