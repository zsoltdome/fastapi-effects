...
def reject_sensitive_keys(value: object, *, path: str = "metadata", depth: int = 0) -> None:
    ...
    if depth > _MAX_DEPTH:
        raise FastAPIEffectsConfigurationError(f"{path} exceeds the maximum nesting depth.")
    if isinstance(value, Mapping):
        if len(value) > _MAX_ITEMS:
            raise FastAPIEffectsConfigurationError(f"{path} contains too many entries.")
        for raw_key, item in value.items():
            if not isinstance(raw_key, str) or not raw_key or len(raw_key) > _MAX_KEY:
                raise FastAPIEffectsConfigurationError(f"{path} contains an invalid key.")
            if is_sensitive_key(raw_key):
                raise FastAPIEffectsConfigurationError(f"{path} contains a sensitive field name.")
