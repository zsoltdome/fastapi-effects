...
class DedupeIdentity:
    ...
    def __post_init__(self) -> None:
        if not isinstance(self.namespace, str) or not _NAMESPACE.fullmatch(self.namespace):
            raise FastAPIEffectsConfigurationError("Dedupe namespace is invalid.")
        if (
            ...
            raise FastAPIEffectsConfigurationError("Dedupe key is invalid.")
        try:
            length = len(self.key.encode("utf-8"))
        except UnicodeError as exc:
            raise FastAPIEffectsConfigurationError("Dedupe key is invalid.") from exc
