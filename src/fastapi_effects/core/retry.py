...
class RetryPolicy:
    ...
    def __post_init__(self) -> None:
        ...
        if base_delay < 0 or maximum_delay < base_delay:
            raise FastAPIEffectsConfigurationError("Retry delay bounds are invalid.")
        if handler_timeout <= 0 or lease_duration <= 0:
            raise FastAPIEffectsConfigurationError("Timeout and lease duration must be positive.")
        if lease_duration <= handler_timeout:
            raise FastAPIEffectsConfigurationError(
                "Lease duration must exceed the handler timeout."
            )
        ...
        if self.jitter != "full":
            raise FastAPIEffectsConfigurationError(
                "The runtime supports only full-jitter retry policy."
            )
    ...
    def retry_delay(self, attempt_number: int, random_source: UniformRandom) -> timedelta:
        ...
        if not _is_positive_integer(attempt_number):
            raise FastAPIEffectsConfigurationError("Retry attempt number must be positive.")
        upper = min(
        ...
        if not math.isfinite(delay) or delay < 0 or delay > upper:
            raise FastAPIEffectsConfigurationError("Random source returned an invalid retry delay.")
        ...
        *,
        ...
        for value in (first_attempt_at, now):
            if value.tzinfo is None or value.utcoffset() is None:
                raise FastAPIEffectsConfigurationError("Retry times must be timezone-aware.")
    ...
    def from_dict(cls, value: dict[str, object]) -> RetryPolicy:
        try:
            ...
            raise FastAPIEffectsConfigurationError("Retry policy snapshot is invalid.") from exc
...
def _finite_number(name: str, value: object) -> float:
    if isinstance(value, bool) or not isinstance(value, (int, float)):
        raise FastAPIEffectsConfigurationError(f"Retry {name} must be numeric.")
    numeric = float(value)
    if not math.isfinite(numeric):
        raise FastAPIEffectsConfigurationError(f"Retry {name} must be finite.")
    return numeric
...
def remaining_attempt_seconds(
    *,
    ...
    if configured_limit_seconds is not None:
        ...
        if configured <= 0:
            raise FastAPIEffectsConfigurationError("Configured attempt limit must be positive.")
...
def _aware_time(name: str, value: datetime) -> None:
    if value.tzinfo is None or value.utcoffset() is None:
        raise FastAPIEffectsConfigurationError(f"Retry {name} time must be timezone-aware.")
