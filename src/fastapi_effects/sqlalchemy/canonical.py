...
_PREFIX = b"fastapi_effects:canonical-json:v1\n"
_MAX_DEPTH = 64
...
def canonical_json_bytes(
    ...
    *,
    ...
    if not isinstance(maximum_bytes, int) or isinstance(maximum_bytes, bool) or maximum_bytes < 1:
        raise FastAPIEffectsConfigurationError("Canonical JSON maximum size must be positive.")
    ...
    if len(encoded) > maximum_bytes:
        raise FastAPIEffectsConfigurationError("Canonical JSON exceeds the configured byte limit.")
    return encoded
...
def versioned_canonical_bytes(
    ...
    *,
    ...
    if body_limit < 1:
        raise FastAPIEffectsConfigurationError("Canonical JSON maximum size is too small.")
    ...
    *,
    ...
    if isinstance(value, str):
        try:
            ...
            raise FastAPIEffectsConfigurationError("JSON input is not valid UTF-8.") from exc
    elif isinstance(value, bytes):
        raw = value
    else:
        raise FastAPIEffectsConfigurationError("JSON input must be bytes or text.")
    if len(raw) > maximum_bytes:
        raise FastAPIEffectsConfigurationError("JSON input exceeds the configured byte limit.")
    ...
    def object_pairs(pairs: list[tuple[str, Any]]) -> dict[str, Any]:
        ...
        for key, item in pairs:
            ...
            if semantic in semantic_keys:
                raise FastAPIEffectsConfigurationError(
                    "JSON object contains duplicate semantic keys."
                )
    ...
    def reject_constant(_value: str) -> None:
        raise FastAPIEffectsConfigurationError("JSON contains a non-finite number.")
    ...
    except FastAPIEffectsConfigurationError:
        raise
    except (UnicodeError, json.JSONDecodeError, RecursionError) as exc:
        raise FastAPIEffectsConfigurationError("JSON input is malformed.") from exc
...
def _normalize(value: object, *, path: str, seen: set[int], depth: int = 0) -> object:
    if depth > _MAX_DEPTH:
        raise FastAPIEffectsConfigurationError("Canonical JSON exceeds the nesting limit.")
    ...
    if value is None or isinstance(value, (str, bool)):
        if isinstance(value, str):
            try:
                ...
                raise FastAPIEffectsConfigurationError(
                    "Canonical JSON contains invalid text."
                ) from exc
        return value
    ...
    if isinstance(value, float):
        if not math.isfinite(value):
            raise FastAPIEffectsConfigurationError("Canonical JSON contains a non-finite number.")
        return value
    if isinstance(value, Mapping):
        if len(value) > _MAX_CONTAINER_ITEMS:
            raise FastAPIEffectsConfigurationError("Canonical JSON object has too many members.")
        identity = id(value)
        if identity in seen:
            raise FastAPIEffectsConfigurationError("Canonical JSON contains a reference cycle.")
        seen.add(identity)
    ...
    if isinstance(value, Sequence) and not isinstance(value, (str, bytes, bytearray)):
        if len(value) > _MAX_CONTAINER_ITEMS:
            raise FastAPIEffectsConfigurationError("Canonical JSON array has too many items.")
        identity = id(value)
        if identity in seen:
            raise FastAPIEffectsConfigurationError("Canonical JSON contains a reference cycle.")
        seen.add(identity)
        try:
            ...
            seen.remove(identity)
    raise FastAPIEffectsConfigurationError(f"Unsupported canonical JSON value at {path}.")
