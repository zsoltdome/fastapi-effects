...
from fastapi_effects.core.protocols import Clock, EffectRouteProvider, EffectStore, UUIDGenerator
...
class FastAPIEffectsUnitOfWork:
    ...
    def __post_init__(self) -> None:
        if not isinstance(self.session, AsyncSession):
            raise FastAPIEffectsConfigurationError(
                "FastAPIEffectsUnitOfWork session must be an AsyncSession."
            )
        if not isinstance(self.principal, Principal):
            raise FastAPIEffectsConfigurationError(
                "FastAPIEffectsUnitOfWork principal must be a Principal."
            )
    ...
    async def __aenter__(self) -> FastAPIEffectsUnitOfWork:
        if self._active or self.session.info.get(_SESSION_MARKER) is not None:
            raise FastAPIEffectsConfigurationError(
                "Nested FastAPIEffects unit of work is not allowed."
            )
        if self.session.in_transaction():
            raise FastAPIEffectsConfigurationError(
                "FastAPIEffects requires an idle session and owns the outer transaction."
            )
    ...
    async def emit(
        ...
        *,
        ...
        if not isinstance(event, Event):
            raise FastAPIEffectsConfigurationError("emit() requires an Event instance.")
        ...
        if not self._active or self._transaction is None:
            raise FastAPIEffectsConfigurationError(
                "emit() requires an active FastAPIEffects unit of work."
            )
...
__all__ = ["FastAPIEffectsUnitOfWork"]
