...
class ReferenceBoundaryDriver:
    ...
    def __init__(self, *, faults: Sequence[Fault | str] = ()) -> None:
        ...
        self._principal: ContextVar[Principal | None] = ContextVar(
            "fastapi_effects_reference_principal", default=None
        )
        self._counter = 0
        ...
        self._manifest = CapabilityManifest(
            adapter_name="FastAPIEffects reference driver",
            adapter_version="1.0.0",
            implementation="fastapi_effects.testing.reference",
    ...
    async def reset(self) -> None:
        ...
        self._principal = ContextVar("fastapi_effects_reference_principal", default=None)
        self._closed = False
    ...
    async def enqueue_handoff(
        ...
        *,
        ...
        handoff_id = uuid5(
            NAMESPACE_URL,
            f"fastapi_effects:handoff:{delivery_id}:{attempt_id}",
        )
        tenant_id = principal.tenant_id
        subject_id = principal.subject_id
        ...
        record = _HandoffRecord(
            ...
            delivery_id=delivery_id,
            attempt_id=attempt_id,
            task_id=f"fastapi_effects_{delivery_id}-{attempt_id}",
    ...
    def _uuid(self, label: str) -> UUID:
        self._counter += 1
        return uuid5(NAMESPACE_URL, f"fastapi_effects:{label}:{self._counter}")
