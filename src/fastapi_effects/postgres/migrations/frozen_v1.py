...
SCHEMA_V1 = "fastapi_effects"
TENANT_EXPRESSION_V1 = (
    "tenant_id = nullif(current_setting('fastapi_effects.tenant_id', true), '')::uuid"
)
...
def role_names_v1(config: Any) -> RoleNamesV1:
    ...
    if configured is None:
        result = RoleNamesV1(
            "fastapi_effects_migration", "fastapi_effects_app", "fastapi_effects_relay"
        )
    else:
        try:
            result = RoleNamesV1(
...
CORE_DDL_V1 = (
    """
    CREATE TABLE fastapi_effects.schema_revision (
    ...
    """,
    """
    CREATE TABLE fastapi_effects.events (
    ...
    """,
    "CREATE INDEX ix_events_tenant_created ON fastapi_effects.events (tenant_id, created_at)",
    """
    CREATE UNIQUE INDEX uq_events_tenant_dedupe
    ON fastapi_effects.events (tenant_id, dedupe_namespace, dedupe_key)
    WHERE dedupe_namespace IS NOT NULL
    """,
    """
    CREATE TABLE fastapi_effects.deliveries (
        ...
        CONSTRAINT fk_deliveries_tenant_id_event_id_events
            ...
            REFERENCES fastapi_effects.events (tenant_id, event_id) ON DELETE RESTRICT,
        CONSTRAINT fk_deliveries_tenant_id_replay_of_deliveries
            FOREIGN KEY(tenant_id, replay_of)
            REFERENCES fastapi_effects.deliveries (tenant_id, delivery_id) ON DELETE RESTRICT,
    ...
    CREATE TABLE fastapi_effects.attempts (
        ...
        CONSTRAINT fk_attempts_tenant_id_delivery_id_deliveries
            ...
            REFERENCES fastapi_effects.deliveries (tenant_id, delivery_id) ON DELETE CASCADE,
    ...
    ON fastapi_effects.attempts (tenant_id, delivery_id, started_at)
    """,
)
...
WEBHOOK_DDL_V1 = (
    """
    CREATE TABLE fastapi_effects.webhook_secret_sets (
    ...
    """,
    """
    CREATE TABLE fastapi_effects.webhook_subscriptions (
    ...
    ON fastapi_effects.webhook_subscriptions (tenant_id, state)
    """,
    """
    CREATE TABLE fastapi_effects.webhook_secret_versions (
        ...
        CONSTRAINT fk_webhook_secret_versions_tenant_id_secret_set_id_webh_2a09
            ...
            REFERENCES fastapi_effects.webhook_secret_sets (tenant_id, secret_set_id)
            ON DELETE CASCADE,
    ...
    ON fastapi_effects.webhook_secret_versions (tenant_id, secret_set_id, state)
    """,
    """
    CREATE TABLE fastapi_effects.webhook_subscription_versions (
        ...
        CONSTRAINT fk_webhook_subscription_versions_tenant_id_subscription_8717
            ...
            REFERENCES fastapi_effects.webhook_subscriptions (tenant_id, subscription_id)
            ON DELETE CASCADE,
    ...
    ON fastapi_effects.webhook_subscription_versions USING gin (exact_event_types)
    """,
    """
    CREATE TABLE fastapi_effects.webhook_audit (
    ...
    ON fastapi_effects.webhook_audit (tenant_id, occurred_at)
    """,
)
...
TASKIQ_DDL_V1 = (
    """
    CREATE TABLE fastapi_effects.taskiq_handoffs (
        ...
        CONSTRAINT fk_taskiq_handoffs_tenant_id_attempt_id_attempts
            ...
            REFERENCES fastapi_effects.attempts (tenant_id, attempt_id) ON DELETE CASCADE,
    ...
    ON fastapi_effects.taskiq_handoffs (state, execution_deadline, updated_at)
    """,
)
...
COMMAND_DDL_V1 = (
    """
    CREATE TABLE fastapi_effects.commands (
    ...
    """,
    "CREATE INDEX ix_commands_retention ON fastapi_effects.commands (state, updated_at)",
    """
    CREATE UNIQUE INDEX uq_commands_current_identity
    ON fastapi_effects.commands (tenant_id, route_id, method, key_digest)
...
def webhook_retention_sql_v1(roles: RoleNamesV1) -> tuple[str, ...]:
    ...
    return (
        ...
        AS $retention$
        BEGIN
            IF nullif(current_setting('fastapi_effects.tenant_id', true), '')
            ...
            PERFORM pg_catalog.pg_advisory_xact_lock(
                pg_catalog.hashtextextended(
                    'fastapi_effects:webhook-retention:' || requested_tenant::text,
                    0
                )
            );
            WITH locked AS (
