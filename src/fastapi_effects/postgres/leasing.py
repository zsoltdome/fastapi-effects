...
from fastapi_effects.core.protocols import RandomSource, UUIDGenerator
from fastapi_effects.core.retry import RetryPolicy, attempt_deadline, delivery_deadline
from fastapi_effects.core.runtime import SystemRandom
...
from fastapi_effects.postgres.time import DatabaseClock, PostgresDatabaseClock
...
class LeaseRepository:
        ...
        *,
        ...
        async with session.begin():
            ...
            for delivery in candidates:
                ...
                if event is None:
                    raise FastAPIEffectsConfigurationError("Delivery references a missing event.")
                await session.flush()
                claimed.append(
                    ClaimedDelivery(
        ...
        *,
        ...
        if not session.in_transaction():
            raise FastAPIEffectsConfigurationError(
                "Transactional success requires an active transaction."
            )
    ...
    async def fail(
        ...
        *,
        ...
        if not isinstance(error, (RetryableDeliveryError, PermanentDeliveryError)):
            raise FastAPIEffectsConfigurationError(
                "Delivery failures must use a safe classified error."
            )
        async with _owned_transaction(session):
    ...
    async def fail_in_transaction(
        ...
        *,
        ...
        if not session.in_transaction():
            raise FastAPIEffectsConfigurationError(
                "Transactional failure requires an active transaction."
            )
        delivery, attempt, finished_at = await _locked_active_rows(
            session,
            claim,
        ...
        if retryable and policy.permits_retry(
            ...
            if retry_after is not None:
                if retry_after.total_seconds() < 0:
                    raise FastAPIEffectsConfigurationError("Retry-After cannot be negative.")
    ...
    async def replay(
        ...
        *,
        ...
        if not isinstance(actor, str) or not actor or len(actor) > 512:
            raise FastAPIEffectsConfigurationError("Replay actor is invalid.")
        if not isinstance(reason, str) or not reason:
            raise FastAPIEffectsConfigurationError("Replay reason is invalid.")
        async with _owned_transaction(session):
    ...
    async def replay_in_transaction(
        ...
        *,
        ...
        if not session.in_transaction():
            raise FastAPIEffectsConfigurationError(
                "Transactional replay requires an active transaction."
            )
        if not isinstance(actor, str) or not actor or len(actor) > 512:
            raise FastAPIEffectsConfigurationError("Replay actor is invalid.")
        if not isinstance(reason, str) or not reason:
        ...
        if destination_kind == "webhook":
            await session.execute(
                text(
                    ...
                    "'fastapi_effects:webhook-retention:' || CAST(:tenant AS text), 0))"
                ),
        ...
        if replay is None:
            raise FastAPIEffectsConfigurationError("Only a terminal delivery can be replayed.")
...
def _snapshot_policy(snapshot: dict[str, Any]) -> RetryPolicy:
    ...
    if not isinstance(retry, dict):
        raise FastAPIEffectsConfigurationError("Delivery retry snapshot is invalid.")
...
def _require_idle(session: AsyncSession) -> None:
    if session.in_transaction():
        raise FastAPIEffectsConfigurationError("Lease operation requires an idle control session.")


def _positive(name: str, value: object) -> None:
    if not isinstance(value, int) or isinstance(value, bool) or value < 1:
        raise FastAPIEffectsConfigurationError(f"Lease {name} must be positive.")
