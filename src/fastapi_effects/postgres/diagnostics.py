    ...
    *,
    ...
    async with engine.connect() as connection:
        ...
        if role_name == configured.application:
            ...
            await connection.execute(
                text("SELECT set_config('fastapi_effects.subject_id', 'doctor', true)")
            )
...
async def _tenant_isolation_probe(connection: AsyncConnection) -> bool:
    ...
    try:
        ...
        await connection.execute(text("SELECT set_config('fastapi_effects.tenant_id', '', true)"))
...
async def _context_settings(connection: AsyncConnection) -> tuple[str | None, str | None]:
    row = (
        await connection.execute(
            text(
                "SELECT nullif(current_setting('fastapi_effects.tenant_id', true), '') AS tenant, "
                "nullif(current_setting('fastapi_effects.subject_id', true), '') AS subject"
            )
        )
    ).one()
