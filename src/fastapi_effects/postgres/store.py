...
class PostgresStore:
    ...
    def __post_init__(self) -> None:
        if not isinstance(self.schema, str) or not _SCHEMA_NAME_PATTERN.fullmatch(self.schema):
            raise FastAPIEffectsConfigurationError("PostgreSQL schema name is invalid.")
        if self.schema != "fastapi_effects":
            raise FastAPIEffectsConfigurationError(
                "The supported PostgreSQL schema is exactly 'fastapi_effects'."
            )
    ...
    async def publish(
        ...
        *,
        ...
        if (dedupe_namespace is None) != (dedupe_key is None):
            raise FastAPIEffectsConfigurationError("Dedupe namespace and key must be paired.")
        identity = (
            None
