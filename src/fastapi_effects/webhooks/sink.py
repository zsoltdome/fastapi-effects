...
from fastapi_effects.webhooks.classification import (
    ResponseDisposition,
    classify_response,
)
from fastapi_effects.webhooks.operations import WebhookHealthObserver
from fastapi_effects.webhooks.secrets import NoEligibleSigningKeyError, WebhookSecretService
from fastapi_effects.webhooks.serializer import (
    serialize_webhook_envelope,
    webhook_message_id,
)
from fastapi_effects.webhooks.signing import sign_webhook
from fastapi_effects.webhooks.transport import (
    AddressResolver,
    ExplicitIPTransport,
...
class WebhookDeliverySink:
    ...
    async def deliver_attempt(self, claim: ClaimedDelivery) -> WebhookAttemptResult:
        ...
        if not isinstance(retry, dict) or lease_expires_at is None:
            raise FastAPIEffectsConfigurationError("Webhook attempt deadline is invalid.")
...
def _destination(snapshot: dict[str, Any]) -> dict[str, Any]:
    ...
    if not isinstance(value, dict):
        raise FastAPIEffectsConfigurationError("Webhook destination snapshot is invalid.")
    return value
...
def _required_uuid(value: dict[str, Any], key: str) -> UUID:
    try:
        ...
        raise FastAPIEffectsConfigurationError("Webhook destination snapshot is invalid.") from exc
