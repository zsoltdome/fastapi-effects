...
from fastapi_effects.webhooks.models import WebhookSecretSetRow, WebhookSecretVersionRow
...
class MasterKey:
    ...
    def __post_init__(self) -> None:
        if not self.key_id or len(self.key_id) > 128:
            raise FastAPIEffectsConfigurationError("Webhook master key identifier is invalid.")
        if len(self.material) not in {16, 24, 32}:
            raise FastAPIEffectsConfigurationError("Webhook AES master key has an invalid size.")
    ...
    async def key_for(self, key_id: str) -> MasterKey:
        for key in self.keys:
            if key.key_id == key_id:
                return key
        raise FastAPIEffectsConfigurationError("Configured webhook master key is unavailable.")
...
class WebhookSecretService:
        ...
        *,
        ...
        if overlap <= timedelta(0) or overlap > timedelta(days=30):
            raise FastAPIEffectsConfigurationError(
                "Webhook secret overlap must be in (0, 30 days]."
            )
        ...
        if active is None:
            raise FastAPIEffectsConfigurationError("Webhook secret set has no active key.")
    ...
    async def revoke(
        ...
        *,
        ...
        if row is None:
            raise FastAPIEffectsConfigurationError("Webhook secret version does not exist.")
        row.state = "revoked"
...
def decode_public_secret(value: str) -> bytes:
    if not isinstance(value, str) or not value.startswith("whsec_"):
        raise FastAPIEffectsConfigurationError("Webhook signing secret is invalid.")
    try:
        decoded = base64.b64decode(value[6:], validate=True)
    except (ValueError, binascii.Error) as exc:
        raise FastAPIEffectsConfigurationError("Webhook signing secret is invalid.") from exc
    if len(decoded) < 16:
        raise FastAPIEffectsConfigurationError("Webhook signing secret is invalid.")
    return decoded
...
def _associated_data(tenant_id: UUID, secret_set_id: UUID, version: int) -> bytes:
    return (
        b"fastapi_effects:webhook-secret:v1\x00"
        + tenant_id.bytes
        + secret_set_id.bytes
...
def _decrypt(
    ...
    *,
    ...
    try:
        ...
        raise FastAPIEffectsConfigurationError(
            "Webhook encrypted secret could not be opened."
        ) from exc


def _aware(value: datetime) -> None:
    if value.tzinfo is None or value.utcoffset() is None:
        raise FastAPIEffectsConfigurationError("Webhook secret time must be timezone-aware.")
