...
def sign_webhook(
    *,
    ...
    if not isinstance(message_id, str) or not _MESSAGE_ID.fullmatch(message_id):
        raise FastAPIEffectsConfigurationError("Webhook message ID is invalid.")
    if timestamp.tzinfo is None or timestamp.utcoffset() is None:
        raise FastAPIEffectsConfigurationError("Webhook signing timestamp must be timezone-aware.")
    if not isinstance(body, bytes):
        raise FastAPIEffectsConfigurationError("Webhook body must be bytes.")
    timestamp_text = str(int(timestamp.timestamp()))
    signed_content = message_id.encode() + b"." + timestamp_text.encode() + b"." + body
    signatures: list[str] = []
    for secret in secrets:
        if not isinstance(secret, bytes) or len(secret) < 16:
            raise FastAPIEffectsConfigurationError("Webhook signing key is invalid.")
        digest = hmac.new(secret, signed_content, hashlib.sha256).digest()
        signatures.append("v1," + base64.b64encode(digest).decode("ascii"))
    if not signatures:
        raise FastAPIEffectsConfigurationError("Webhook signing requires an eligible key.")
...
def verify_webhook(
    *,
    ...
    if not isinstance(secret, bytes) or len(secret) < 16:
        raise FastAPIEffectsConfigurationError("Webhook verification key is invalid.")
    if not isinstance(body, bytes):
        raise FastAPIEffectsConfigurationError("Webhook verification body must be bytes.")
    current = now or datetime.now(UTC)
    if current.tzinfo is None or current.utcoffset() is None:
        raise FastAPIEffectsConfigurationError("Webhook verification time must be timezone-aware.")
    for name, tolerance in (
        ...
        if (
            ...
            raise FastAPIEffectsConfigurationError(f"Webhook {name} is invalid.")
