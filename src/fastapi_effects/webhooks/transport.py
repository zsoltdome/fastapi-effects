...
from fastapi_effects.webhooks.address_policy import (
    EndpointTarget,
    parse_endpoint,
    validate_public_addresses,
)
from fastapi_effects.webhooks.http11 import (
...
async def resolve_endpoint(
    ...
    *,
    ...
    if (
        ...
        raise FastAPIEffectsConfigurationError("Webhook address attempt limit is invalid.")
    if production:
        addresses = validate_public_addresses(list(addresses))
    elif not addresses:
        raise FastAPIEffectsConfigurationError("Webhook DNS resolver returned no addresses.")
    ...
    *,
    ...
    for name, value in sorted(headers.items(), key=lambda item: item[0].lower()):
        ...
        if (
            ...
            raise FastAPIEffectsConfigurationError("Webhook request headers are invalid.")
        lines.append(f"{name}: {value}")
    lines.extend((f"Content-Length: {len(body)}", "Connection: close", "", ""))
    try:
        head = "\r\n".join(lines).encode("ascii")
    except UnicodeEncodeError as exc:
        raise FastAPIEffectsConfigurationError("Webhook request headers must be ASCII.") from exc
    return head + body
...
def _validate_ssl_context(context: ssl.SSLContext) -> None:
    ...
    if int(context.minimum_version) < int(ssl.TLSVersion.TLSv1_2):
        raise FastAPIEffectsConfigurationError("Production webhook TLS requires TLS 1.2 or newer.")
    maximum = context.maximum_version
    if maximum != ssl.TLSVersion.MAXIMUM_SUPPORTED and int(maximum) < int(ssl.TLSVersion.TLSv1_2):
        raise FastAPIEffectsConfigurationError("Production webhook TLS maximum excludes TLS 1.2.")
