    ...
    def __post_init__(self) -> None:
        for value in (
            ...
            if value is not None and not isinstance(value, UUID):
                raise FastAPIEffectsConfigurationError("Runtime trace lineage identity is invalid.")
        if self.traceparent is not None and (
            ...
            raise FastAPIEffectsConfigurationError("Runtime trace lineage parent is invalid.")
...
class RuntimeEvent:
    ...
    def __post_init__(self) -> None:
        if not isinstance(self.kind, RuntimeEventKind):
            raise FastAPIEffectsConfigurationError("Runtime event kind is invalid.")
        if self.occurred_at.tzinfo is None or self.occurred_at.utcoffset() is None:
            raise FastAPIEffectsConfigurationError("Runtime event time must be timezone-aware.")
        if len(self.attributes) > 32:
            raise FastAPIEffectsConfigurationError("Runtime event has too many attributes.")
        ...
        for key, value in self.attributes.items():
            if (
                ...
                raise FastAPIEffectsConfigurationError(
                    "Runtime event contains an unsafe attribute."
                )
            if not isinstance(value, (str, int, float, bool)):
                raise FastAPIEffectsConfigurationError(
                    "Runtime event attribute type is unsupported."
                )
            if isinstance(value, float) and not math.isfinite(value):
                raise FastAPIEffectsConfigurationError("Runtime event attribute must be finite.")
            if isinstance(value, str) and len(value) > 256:
                raise FastAPIEffectsConfigurationError("Runtime event attribute is oversized.")
            normalized[key] = value
        object.__setattr__(self, "attributes", MappingProxyType(normalized))
        if not isinstance(self.lineage, TraceLineage):
            raise FastAPIEffectsConfigurationError("Runtime event trace lineage is invalid.")
