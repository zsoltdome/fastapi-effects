...
from fastapi_mergen.observability import EventSink, NoOpEventSink, RuntimeEvent, RuntimeEventKind
...
from fastapi_mergen.webhooks.models import (
    WebhookAuditRow,
    WebhookSubscriptionRow,
)
from fastapi_mergen.webhooks.secrets import CreatedWebhookSecret, WebhookSecretService
...
class WebhookHealthRepository:
    ...
    async def _locked(
        ...
        if row is None:
            raise MergenConfigurationError("Webhook health target no longer exists.")
        return row
    ...
    async def retain(
        ...
        *,
        ...
        if session.in_transaction():
            raise MergenConfigurationError("Webhook retention requires an idle session.")
        if before.tzinfo is None or before.utcoffset() is None:
            raise MergenConfigurationError("Webhook retention cutoff must be timezone-aware.")
        if not 1 <= batch_size <= 10_000:
            raise MergenConfigurationError("Webhook retention batch size is invalid.")
            ...
            row = (
                await session.execute(
                    text(
                        ...
                        "fastapi_mergen.prune_webhook_history(:tenant, :cutoff, :batch_size)"
                    ),
                    {
...
def require_webhook_management(principal: Principal) -> None:
    if not ({"webhooks:manage", "mergen:operator"} & principal.scopes):
