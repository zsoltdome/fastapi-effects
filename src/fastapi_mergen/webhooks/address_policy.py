...
def parse_endpoint(
    ...
    *,
    ...
    if (
        ...
        raise MergenConfigurationError("Webhook endpoint URL is invalid.")
    try:
        parsed = urlsplit(value)
        port = parsed.port
    except ValueError as exc:
        raise MergenConfigurationError("Webhook endpoint authority is invalid.") from exc
    allowed_schemes = {"https"} if production else {"http", "https"}
    if parsed.scheme.lower() not in allowed_schemes:
        raise MergenConfigurationError("Webhook endpoint scheme is not allowed.")
    if (
        not parsed.netloc
        ...
        raise MergenConfigurationError("Webhook endpoint authority is invalid.")
    try:
        hostname = parsed.hostname.encode("idna").decode("ascii").lower().rstrip(".")
    except UnicodeError as exc:
        raise MergenConfigurationError("Webhook endpoint hostname is invalid.") from exc
    if not hostname or len(hostname) > 253:
        raise MergenConfigurationError("Webhook endpoint hostname is invalid.")
    effective_port = port or (443 if parsed.scheme.lower() == "https" else 80)
    if effective_port not in allowed_ports:
        raise MergenConfigurationError("Webhook endpoint port is not allowed.")
