...
from fastapi_mergen.core.context import principal_context
...
from fastapi_mergen.core.protocols import AuthorizationResolver, Clock, HandlerSessionProvider
from fastapi_mergen.core.retry import RetryPolicy
from fastapi_mergen.core.routing import RouteRegistry
...
from fastapi_mergen.sqlalchemy.canonical import strict_json_loads

_PAYLOAD_PREFIX = b"fastapi-mergen:canonical-json:v1\n"
...
class HandlerExecutor:
    def __init__(
        ...
        *,
        ...
        if not registry.frozen:
            raise MergenConfigurationError("Handler registry must be frozen before execution.")
    ...
    async def execute(self, claim: ClaimedDelivery) -> None:
        ...
        if not isinstance(destination, dict) or not isinstance(authority, dict):
            raise MergenConfigurationError("Handler delivery snapshot is invalid.")
        if not isinstance(retry_value, dict):
            raise MergenConfigurationError("Handler retry snapshot is invalid.")
        ...
        if not body.startswith(_PAYLOAD_PREFIX):
            raise MergenConfigurationError("Event canonical payload revision is unsupported.")
    ...
    async def _effective_principal(
        ...
        try:
            ...
            raise MergenConfigurationError("Authority snapshot is invalid.") from exc
        if mode is AuthorizationMode.SNAPSHOT:
            ...
            if not isinstance(maximum_age, int) or isinstance(maximum_age, bool):
                raise MergenConfigurationError("Snapshot authority age is invalid.")
            ...
            if self._authorization_resolver is None:
                raise MergenConfigurationError("Revalidation requires an authorization resolver.")
            ...
            if not isinstance(policy, str) or capabilities is None:
                raise MergenConfigurationError("Service authority snapshot is invalid.")
...
def _text(value: object) -> str:
    if not isinstance(value, str):
        raise MergenConfigurationError("Handler destination is invalid.")
    return value


def _string_list(value: object) -> tuple[str, ...]:
    if not isinstance(value, list) or any(not isinstance(item, str) for item in value):
        raise MergenConfigurationError("Authority scope snapshot is invalid.")
    return tuple(value)
