...
class DelegationClaims:
    ...
    def __post_init__(self) -> None:
        if self.version != 1:
            raise MergenConfigurationError("Delegation claim version is unsupported.")
        for name, value in (
            ("issuer", self.issuer),
            ...
            if not isinstance(value, str) or not _IDENTIFIER.fullmatch(value):
                raise MergenConfigurationError(f"Delegation {name} is invalid.")
        ...
        for optional_name, optional_value in optional_identities:
            if optional_value is not None and (
                ...
                raise MergenConfigurationError(f"Delegation {optional_name} is invalid.")
        if not isinstance(self.tenant_id, UUID) or not isinstance(self.token_id, UUID):
            raise MergenConfigurationError("Delegation UUID identity is invalid.")
        if not isinstance(self.key_id, str) or not _KEY_ID.fullmatch(self.key_id):
            raise MergenConfigurationError("Delegation key ID is invalid.")
        ...
        if isinstance(scope_value, str) or not isinstance(scope_value, tuple):
            raise MergenConfigurationError("Delegation scopes must be a collection.")
        if len(self.scopes) > 128 or any(
            not isinstance(scope, str) or not _SCOPE.fullmatch(scope) for scope in self.scopes
        ):
            raise MergenConfigurationError("Delegation scope set is invalid.")
        ...
        for name, timestamp in (
            ...
            if (
                ...
                raise MergenConfigurationError(f"Delegation {name} must be timezone-aware.")
        if self.not_before < self.issued_at or self.expires_at <= self.not_before:
            raise MergenConfigurationError("Delegation validity interval is invalid.")
        if (
            self.expires_at - self.issued_at
        ).total_seconds() > _MAXIMUM_CREDENTIAL_LIFETIME_SECONDS:
            raise MergenConfigurationError("Delegation credential lifetime is excessive.")
        if (
            not isinstance(self.delegation_depth, int)
            or isinstance(self.delegation_depth, bool)
            or not 1 <= self.delegation_depth <= 16
        ):
            raise MergenConfigurationError("Delegation depth is invalid.")
    ...
    def from_dict(cls, value: Mapping[str, object]) -> DelegationClaims:
        if not isinstance(value, Mapping) or set(value) != _FIELDS:
            raise MergenConfigurationError("Delegation claims are incomplete or unknown.")
        scopes = value["scopes"]
        if not isinstance(scopes, list) or any(not isinstance(item, str) for item in scopes):
            raise MergenConfigurationError("Delegation scopes are invalid.")
        try:
            return cls(
            ...
            raise MergenConfigurationError("Delegation claims are invalid.") from exc
