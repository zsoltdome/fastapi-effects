...
def canonical_method(value: str) -> str:
    if not isinstance(value, str) or not _METHOD.fullmatch(value):
        raise MergenConfigurationError("Delegation HTTP method is invalid.")
    return value
...
def canonical_target_path(value: str) -> str:
    ...
    if (
        ...
        raise MergenConfigurationError("Delegation target path is invalid.")
    try:
        value.encode("ascii")
    ...
    while index < len(value):
        ...
        if index + 2 >= len(value) or value[index + 1] not in _HEX or value[index + 2] not in _HEX:
            raise MergenConfigurationError("Delegation target has malformed percent encoding.")
        octet = int(value[index + 1 : index + 3], 16)
        decoded = chr(octet)
        if decoded in {"/", "\\", "%"} or octet < 0x20 or octet == 0x7F:
            raise MergenConfigurationError("Delegation target contains an unsafe encoded octet.")
    ...
    if any(segment in {"", ".", ".."} for segment in segments):
        raise MergenConfigurationError("Delegation target path contains an ambiguous segment.")
    return result
