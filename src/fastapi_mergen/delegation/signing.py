...
from fastapi_mergen.delegation.encoding import claims_bytes, decode_token, encode_token
from fastapi_mergen.delegation.models import DelegationClaims
from fastapi_mergen.delegation.protocols import DelegationAuditSink, DelegationKeyRing
from fastapi_mergen.delegation.targets import canonical_method, canonical_target_path
    ...
    def mint(
        ...
        *,
        ...
        if not isinstance(principal, Principal):
            raise MergenConfigurationError("Delegation requires a trusted principal.")
        if not isinstance(scopes, frozenset) or not scopes.issubset(principal.scopes):
            raise MergenConfigurationError("Delegation scopes exceed the verified principal.")
        if (
            not isinstance(parent_depth, int)
            or isinstance(parent_depth, bool)
            or not 0 <= parent_depth < self.maximum_depth
        ):
            raise MergenConfigurationError("Delegation parent depth is invalid.")
        ...
        if seconds <= 0 or seconds > self.maximum_ttl_seconds or not seconds.is_integer():
            raise MergenConfigurationError("Delegation TTL is invalid.")
        depth = parent_depth + 1
        if depth > self.maximum_depth:
            raise MergenConfigurationError("Delegation depth exceeds configured policy.")
        now = self.clock.now()
        expiry = now + ttl
        if principal.expires_at is not None and expiry > principal.expires_at:
            expiry = principal.expires_at
        if expiry <= now:
            raise MergenConfigurationError("Delegation principal is expired.")
...
class DelegationVerifier:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise MergenConfigurationError("Delegation verifier clock skew is invalid.")
        if self.allowed_scopes is not None and (
            ...
            raise MergenConfigurationError("Delegation verifier scope ceiling is invalid.")
...
def _configuration(issuer: object, maximum_ttl_seconds: object, maximum_depth: object) -> None:
    if not isinstance(issuer, str) or not _IDENTIFIER.fullmatch(issuer):
        raise MergenConfigurationError("Delegation issuer configuration is invalid.")
    if (
        not isinstance(maximum_ttl_seconds, int)
        or isinstance(maximum_ttl_seconds, bool)
        or not 1 <= maximum_ttl_seconds <= 3600
    ):
        raise MergenConfigurationError("Delegation maximum TTL is invalid.")
    if (
        not isinstance(maximum_depth, int)
        or isinstance(maximum_depth, bool)
        or not 1 <= maximum_depth <= 16
    ):
        raise MergenConfigurationError("Delegation maximum depth is invalid.")
