...
from fastapi_mergen.delegation.audit import (
...
class SigningKey:
    ...
    def __post_init__(self) -> None:
        if not isinstance(self.key_id, str) or not _KEY_ID.fullmatch(self.key_id):
            raise MergenConfigurationError("Delegation signing key ID is invalid.")
        if not isinstance(self.secret, bytes) or not 32 <= len(self.secret) <= 128:
            raise MergenConfigurationError("Delegation signing key material is invalid.")
        if not isinstance(self.state, SigningKeyState):
            raise MergenConfigurationError("Delegation signing key state is invalid.")
        for value in (self.retired_at, self.verify_until):
            if value is not None and (
                ...
                raise MergenConfigurationError("Delegation key retirement time is invalid.")
        if self.state is SigningKeyState.RETIRING and (
            ...
            raise MergenConfigurationError("A retiring key requires a valid overlap interval.")
        if self.state is not SigningKeyState.RETIRING and (
            self.retired_at is not None or self.verify_until is not None
        ):
            raise MergenConfigurationError("Only retiring keys accept retirement times.")
...
class InMemoryKeyRing:
    ...
    def __init__(
        ...
        *,
        ...
        if not keys or len(keys) > 16:
            raise MergenConfigurationError("Delegation key ring size is invalid.")
        by_id = {key.key_id: key for key in keys}
        if len(by_id) != len(keys):
            raise MergenConfigurationError("Delegation key ring repeats a key ID.")
        if sum(key.state is SigningKeyState.ACTIVE for key in keys) != 1:
            raise MergenConfigurationError("Delegation key ring requires exactly one active key.")
        if (
            not isinstance(maximum_overlap, timedelta)
            or maximum_overlap < timedelta(seconds=1)
            or maximum_overlap > timedelta(hours=1)
        ):
            raise MergenConfigurationError("Delegation key overlap policy is invalid.")
        if any(
            ...
            raise MergenConfigurationError("Delegation retiring key overlap is excessive.")
        self._keys = by_id
    ...
    def signing_key(self, now: datetime) -> SigningKey:
        ...
        if len(active) != 1:
            raise MergenConfigurationError("Delegation signing is disabled without an active key.")
        return active[0]
    ...
    def rotate(
        ...
        *,
        ...
        if new_key.state is not SigningKeyState.ACTIVE or new_key.key_id in self._keys:
            raise MergenConfigurationError("Delegation rotation key is invalid or repeated.")
        ...
        if (
            ...
            raise MergenConfigurationError("Delegation rotation overlap is invalid.")
    ...
    def revoke(
        ...
        *,
        ...
        key = self._keys.get(key_id)
        if key is None:
            raise MergenConfigurationError("Delegation revocation key is unknown.")
...
def _aware(value: datetime) -> None:
    if not isinstance(value, datetime) or value.tzinfo is None or value.utcoffset() is None:
        raise MergenConfigurationError("Delegation key operation time must be timezone-aware.")
