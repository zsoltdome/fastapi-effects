...
from fastapi_mergen.postgres.rls import rls_sql
from fastapi_mergen.postgres.roles import RuntimeRoles, create_role_sql
...
async def drop_core_schema(engine: AsyncEngine) -> None:
    """Drop only the authoritative Mergen schema (primarily for isolated tests)."""
    async with engine.begin() as connection:
