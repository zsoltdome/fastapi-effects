...
from fastapi_mergen.idempotency.fingerprint import (
...
async def prepare_request(
    ...
    *,
    ...
    if not isinstance(request, Request):
        raise MergenConfigurationError("Command preparation requires a Starlette request.")
    ...
    if key is None:  # Defensive: required headers are rejected by _one_raw_header.
        raise MergenConfigurationError("Idempotency-Key is required.")
    ...
    for configured in representation_headers:
        ...
        if not _HEADER_NAME.fullmatch(name) or name in normalized_names:
            raise MergenConfigurationError("Command representation header name is invalid.")
    ...
    if content_type is not None:
        try:
            ...
            raise MergenConfigurationError("Command media type is invalid.") from exc
    path_parameters = {str(name): str(value) for name, value in request.path_params.items()}
    raw_query = request.scope.get("query_string", b"")
    if not isinstance(raw_query, bytes):
        raise MergenConfigurationError("Command query representation is invalid.")
...
async def _bounded_body(request: Request, *, maximum_body_bytes: int) -> bytes:
    ...
    if declared is not None:
        try:
            ...
            raise MergenConfigurationError("Command Content-Length is invalid.") from exc
...
def _one_raw_header(
    ...
    *,
    ...
    if len(values) > 1 or (required and not values):
        raise MergenConfigurationError("Required command request header is missing or repeated.")
