...
class CapturedResponse:
    ...
    def __post_init__(self) -> None:
        if (
            ...
            raise MergenConfigurationError("Command response status is not replay-safe.")
        if not isinstance(self.media_type, str):
            raise MergenConfigurationError("Command response media type is not replay-safe.")
        ...
        if not isinstance(self.body, bytes) or len(self.body) > MAX_RESPONSE_BODY_BYTES:
            raise MergenConfigurationError("Command response body is oversized.")
        normalized: dict[str, str] = {}
        size = 0
        if not isinstance(self.headers, Mapping):
            raise MergenConfigurationError("Command response headers are invalid.")
        for raw_name, value in self.headers.items():
            if not isinstance(raw_name, str):
                raise MergenConfigurationError("Command response header is not replay-safe.")
            name = raw_name.lower()
            if (
        ...
        if size > MAX_RESPONSE_HEADER_BYTES:
            raise MergenConfigurationError("Command response headers are oversized.")
...
def capture_response(response: Response) -> CapturedResponse:
    ...
    if not isinstance(body, bytes):
        raise MergenConfigurationError("Streaming responses cannot be replayed.")
    ...
    for name, value in response.headers.items():
        ...
        if normalized in _UNSAFE_HEADERS:
            raise MergenConfigurationError("Command response contains an unsafe header.")
        headers[name] = value
