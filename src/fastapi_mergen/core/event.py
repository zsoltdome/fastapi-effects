...
from fastapi_mergen.core.principal import PrincipalEnvelope
...
class EventRecord:
    ...
    def __post_init__(self) -> None:
        if not isinstance(self.event_id, UUID) or not isinstance(self.tenant_id, UUID):
            raise MergenConfigurationError("Event record identities must be UUID values.")
        Event(
        ...
        if not _is_positive_integer(self.canonical_version):
            raise MergenConfigurationError("Event canonical version must be positive.")
        if not isinstance(self.payload_canonical, bytes) or not self.payload_canonical:
            raise MergenConfigurationError("Event canonical payload must be non-empty bytes.")
        if not isinstance(self.payload_sha256, bytes) or len(self.payload_sha256) != 32:
            raise MergenConfigurationError("Event payload digest must be SHA-256 bytes.")
        if not isinstance(self.principal, PrincipalEnvelope):
            raise MergenConfigurationError("Event record principal is invalid.")
        if self.principal.tenant_id != self.tenant_id:
            raise MergenConfigurationError("Event tenant and principal tenant must match.")
        if (
            not isinstance(self.created_at, datetime)
            or self.created_at.tzinfo is None
            or self.created_at.utcoffset() is None
        ):
            raise MergenConfigurationError("Event created_at must be timezone-aware.")
        if (self.dedupe_namespace is None) != (self.dedupe_key is None):
            raise MergenConfigurationError("Event dedupe fields must be provided together.")
...
def _validate_traceparent(value: object) -> None:
    ...
    if (
        ...
        raise MergenConfigurationError("Event traceparent is invalid.")
...
def _validate_tracestate(value: object) -> None:
    ...
    if (
        ...
        raise MergenConfigurationError("Event tracestate is invalid.")
    members = value.split(",")
    if len(members) > 32:
        raise MergenConfigurationError("Event tracestate is invalid.")
    seen: set[str] = set()
