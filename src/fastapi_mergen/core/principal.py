...
class PrincipalEnvelope:
    ...
    def from_dict(cls, value: Mapping[str, object]) -> PrincipalEnvelope:
        if not isinstance(value, Mapping):
            raise MergenConfigurationError("Principal envelope must be an object.")
        keys = set(value)
        if keys & _RAW_CREDENTIAL_FIELDS or keys != _ENVELOPE_FIELDS:
            raise MergenConfigurationError("Principal envelope contains unsupported fields.")
        scopes = value["scopes"]
        if isinstance(scopes, str) or not isinstance(scopes, (list, tuple)):
            raise MergenConfigurationError("Principal envelope scopes are invalid.")
        try:
            return cls(
            ...
            raise MergenConfigurationError("Principal envelope is invalid.") from exc
...
def _require_string(value: object) -> str:
    if not isinstance(value, str):
        raise MergenConfigurationError("Principal envelope field must be text.")
    return value
