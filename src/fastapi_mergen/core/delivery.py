    ...
    def __post_init__(self) -> None:
        for identity in (self.delivery_id, self.tenant_id, self.event_id):
            if not isinstance(identity, UUID):
                raise MergenConfigurationError("Delivery identities must be UUID values.")
        for name, key in (
            ...
            if not isinstance(key, str) or not _KEY.fullmatch(key):
                raise MergenConfigurationError(f"Delivery {name} is invalid.")
        if not _positive(self.route_version):
            raise MergenConfigurationError("Delivery route version must be positive.")
        if not isinstance(self.attempts_started, int) or isinstance(self.attempts_started, bool):
            raise MergenConfigurationError("Delivery attempts_started is invalid.")
        if self.attempts_started < 0:
            raise MergenConfigurationError("Delivery attempts_started cannot be negative.")
        if not isinstance(self.route_snapshot, bytes) or not self.route_snapshot:
            raise MergenConfigurationError("Delivery route snapshot must be immutable bytes.")
        if not isinstance(self.state, DeliveryState):
            raise MergenConfigurationError("Delivery state is invalid.")
        for name, value in (
        ...
        if (self.lease_token is None) != (self.lease_expires_at is None):
            raise MergenConfigurationError("Delivery lease token and expiry must be paired.")
        if self.state is DeliveryState.LEASED and self.lease_token is None:
            raise MergenConfigurationError("A leased delivery requires a lease token.")
        if self.state is not DeliveryState.LEASED and self.lease_token is not None:
            raise MergenConfigurationError("Only a leased delivery may retain a lease token.")
        if self.replay_of is None:
            ...
            raise MergenConfigurationError("Replay requires a bounded actor and reason.")
        if self.updated_at < self.created_at:
            raise MergenConfigurationError("Delivery updated before it was created.")
...
class AttemptRecord:
    ...
    def __post_init__(self) -> None:
        for identity in (self.attempt_id, self.tenant_id, self.delivery_id, self.lease_token):
            if not isinstance(identity, UUID):
                raise MergenConfigurationError("Attempt identities must be UUID values.")
        if not _positive(self.attempt_number):
            raise MergenConfigurationError("Attempt number must be positive.")
        if not isinstance(self.outcome, AttemptOutcome):
            raise MergenConfigurationError("Attempt outcome is invalid.")
        ...
        if self.outcome is AttemptOutcome.STARTED:
            if (
                ...
                raise MergenConfigurationError("A started attempt cannot contain an outcome.")
        elif self.finished_at is None:
            raise MergenConfigurationError("A completed attempt requires finished_at.")
        if self.outcome is AttemptOutcome.SUCCEEDED and (
            self.failure_code is not None or self.failure_summary is not None
        ):
            raise MergenConfigurationError("A successful attempt cannot contain a failure.")
        if self.outcome not in {AttemptOutcome.STARTED, AttemptOutcome.SUCCEEDED} and (
            ...
            raise MergenConfigurationError("A failed attempt requires bounded failure metadata.")
        if self.finished_at is not None:
            _aware("finished_at", self.finished_at)
            if self.finished_at < self.started_at:
                raise MergenConfigurationError("Attempt finished before it started.")
        if self.failure_code is not None and (
            not isinstance(self.failure_code, str) or not _KEY.fullmatch(self.failure_code)
        ):
            raise MergenConfigurationError("Attempt failure code is invalid.")
        if self.failure_summary is not None and (
            not isinstance(self.failure_summary, str) or len(self.failure_summary) > 512
        ):
            raise MergenConfigurationError("Attempt failure summary is invalid.")
...
def _aware(name: str, value: object) -> None:
    if not isinstance(value, datetime) or value.tzinfo is None or value.utcoffset() is None:
        raise MergenConfigurationError(f"Delivery {name} must be timezone-aware.")
