...
def test_canonical_payload_order_and_digest_are_stable() -> None:
    ...
    assert versioned_canonical_bytes(left).startswith(b"fastapi-mergen:canonical-json:v1\n")
...
def test_strict_json_rejects_duplicate_semantic_keys_and_bounds() -> None:
    with pytest.raises(MergenConfigurationError, match="duplicate semantic"):
        strict_json_loads('{"é":1,"é":2}')
    with pytest.raises(MergenConfigurationError, match="byte limit"):
