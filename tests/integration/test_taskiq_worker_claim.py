...
from fastapi_effects.errors import LeaseLost
from fastapi_effects.executors.taskiq.adapter import (
    TaskiqDeliverySink,
...
from fastapi_effects.executors.taskiq.store import ExecutingHandoff, TaskiqHandoffStore
...
async def test_real_taskiq_broker_duplicate_is_fenced_and_context_is_reset(
    ...
    async def handler(context: EffectContext[object]) -> None:
        ...
        async with context.application_session() as session:
            tenant = await session.scalar(
                text("SELECT current_setting('fastapi_effects.tenant_id')")
            )
            subject = await session.scalar(
                text("SELECT current_setting('fastapi_effects.subject_id')")
            )
    ...
    fastapi_effects.route(event_type="invoice.taskiq", route_key="invoice.taskiq").to_handler(
        handler,
...
async def test_expired_execution_rejects_stale_finalization(
    ...
    fastapi_effects = FastAPIEffects(
        principal_provider=unused_principal_provider, store=PostgresStore()
    )
    fastapi_effects.route(event_type="expiry.test", route_key="expiry.test").to_handler(
        noop,
        authorization=AuthorizationMode.SNAPSHOT,
        maximum_snapshot_age_seconds=3600,
        retry_policy=RetryPolicy(
...
async def test_old_handoff_is_rejected_after_parent_attempt_is_reclaimed(
    ...
    fastapi_effects.route(event_type="stale.test", route_key="stale.test").to_handler(
        noop,
        authorization=AuthorizationMode.SNAPSHOT,
        maximum_snapshot_age_seconds=3600,
        retry_policy=RetryPolicy(
